...
- 顶栏：章节标记、章节标题、章节进度、自动播放开关、音乐与音效开关
- 剧情面板：正文阅读区 + 选项按钮（同一块底）
- AI 状态面板
- 可选：章节进度，例如 `03 / 06`

#### 布局

先上下切，再左右分。顶栏横跨整个舞台，下面才是两列。

```txt
┌──────────────────────────────────────────────┐
│ CHAPTER 01 / 效率焦虑        进度 · 开关组     │  顶栏
│ 第一章：效率焦虑                              │
├───────────────────────────────┬──────────────┤
│ 正文阅读区（内部滚动）          │  AI 状态面板  │
│                               │              │
...
- 舞台最大宽度约 1080px，正文列约 680–760px；
- 状态面板在右侧，宽度约 260–300px；
- 正文与选项在**同一块面板**里，滚动条落在面板内侧，不贴面板外沿；
- 音频开关排在顶栏右端，不悬浮在正文之上。
...
- 桌面端横向网格。正式剧情固定四条，排成 2×2；
  同一批按钮尺寸完全一致，文案放不下时在按钮内部折行；
- 移动端回到单列全宽：窄屏上 2×2 每列只剩九个字，比竖排更难读。
...
桌面端示例（每行是「图标 + 中文标签 / 英文副标」在左、状态文案在右）：
...
移动端只保留中文标签与状态文案，图标与英文副标隐藏。

#### 视觉

- 标题与运行提示并成一行，提示前面一颗小圆点表示实时跟随；
- 状态项使用短标签：中文标签最多两字，状态文案统一收在四字以内，
  面板右列才对得齐、也不会折行；
- 图标只区分「这是哪一项」，不表达数值高低；
- 可以使用很细的分割线；
- 不使用大进度条展示裸数值；
...
页面上不出现变量裸数字、规则 ID 或节点／选项 ID。
这些只在控制台打印一次（`utils/story/endingSummaryLog.ts`），供开发核对。

#### 构图

桌面端一屏三块，整页不滚动（对齐 `design/ui-mockups/ui-ending-desktop.webp`）：

```txt
┌──────────────────────────────────────┐
│           结局标题 + 副标题            │
├──────────────────┬───────────────────┤
│ 结局正文（内部滚动）│ AI 镜像报告（内部滚动）│
├──────────────────┴───────────────────┤
│ 状态摘要：四项，图标 + 档位点 + 状态文案 │
├──────────────────────────────────────┤
│  [复制我的镜像报告]   [重新初始化]      │
...
两块文本各自独立滚动，整页高度固定：状态摘要与「重新初始化」始终在视野里，
玩家不必翻到底才能重来。状态摘要要压得比参考图更矮 ——
正式剧情的结局正文与镜像报告都远比参考图里长，高度优先给上面那两块。

移动端回到一条正常滚动的长页（对齐 `ui-ending-mobile.webp`）：

- 单列，两块文本上下排，各自完整展开、不再内部滚动；
- 先结局文案，后镜像报告，再状态摘要（2×2）；
- 操作按钮全宽上下排列。

窄屏上并排每列只剩约 160px，上下排又只分到约 144px 的可读高度，
两种「一屏三块」的变体都不成立，所以这里整个解除固定舞台。
...
- 高度不低于 44px；
- 圆角要小：面板 12px、卡片 9px、按钮 7px（`--radius-panel` / `--radius-card`
  / `--radius-button`）。这套界面的语气是安静的仪器面板，边角应该几乎是直的，
  只做一点点倒角；12–16px 那一档在这个尺寸下会圆出明显的弧，整体显得钝。
  胶囊开关是例外：999px 是它的形状本身，不是倒角；
- 边框细；
...
- 标签、状态文案与图标都与剧情页的 AI 状态面板一致，读同一份映射
  （`src/utils/aiStatus.ts`）——玩家在通关那一刻看到的词，和他一路上看到的是同一批词；
- 那排点画的是**第几档**（共五档），不是变量值：变量没有上下限，
  也就没有可以归一化的量程，更不是百分比；
- 点是纯视觉，`aria-hidden`；右边的状态文案已经把同一件事说完整了。
